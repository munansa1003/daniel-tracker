...
| **R-29** | 사용자 판단 필요 | `adaptiveOn`을 끄면 `tdeeHistory`가 통째로 무시돼(`App.jsx:706`) **과거 날짜의 판정까지 보정 없음으로 바뀐다.** 켜면 되살아난다. 코드가 명시한 의도("적응형 보정은 `from` 날짜로 과거를 보존")와 어긋나므로 고치는 것이 맞지만, 고치는 순간 **지금 꺼둔 상태라면 과거 판정 표시가 달라진다** — 사용자가 보게 될 변화라 의도를 확인하고 진행할 일이다 |
...
| 이번 배치에서 수정 | 4 | R-18 · R-19 · R-32 · R-37 |
| 문서화로 처리 | 3 | R-15 · R-17 · R-45 |
| 기각 | 1 | R-24 |
| 사용자 판단 필요 | 3 | R-29 · R-36 · R-38 |
| **합계** | **50** | 중복 0 · 누락 0 (스크립트 대조) |

**수정 43건 · 문서화 3건 · 기각 1건 · 판단 대기 3건.**
