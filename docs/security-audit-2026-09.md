...
| 경로 | 비초대 계정이 할 수 있는 것 |
|---|---|
| `share-create` | 600KB 스냅샷을 IP당 분당 10건 저장(TTL 최대 7일) → **Upstash 무료 256MB를 약 44분에 소진** |
| `share-create` | 서비스 도메인 `/export/view/<token>`에 임의 텍스트를 7일간 호스팅(피싱 문구·스팸) |
| `push-sync` | `push:uids`에 등록 → 매일 밤 크론이 그 계정을 처리(→ P0-4와 결합) |
| `import-inbox` | 서버 설정 상태 노출(`enabled`·`bodyCloudEnabled`·컷오버 날짜), ack로 KV 명령 소모 |
...
→ `SMEMBERS` 순서상 **그 뒤의 모든 사용자가 그날 푸시를 받지 못한다.** 200을 돌려주면 404/410
자동 정리도 발동하지 않아 영구히 반복된다. 로그에는 원인이 남지 않는다.

이건 공격 없이도 발생한다: 파일 상단 주석(`:11-14`)이 이미 "구독자가 늘면 잘려서 뒤쪽 uid가
조용히 누락된다"고 인지하고 있다. uid당 0.3~1.5초로 보면 **약 40~200명부터** 자연 발생한다.

부수적으로 Vercel 함수가 임의 외부 URL로 VAPID 서명 헤더가 붙은 POST를 보낸다(SSRF 성격).
다만 VAPID의 `aud`가 endpoint origin에 묶여 재사용이 불가하고 Vercel에 내부망이 없어 정보 가치는 낮다.
...
**영향**: 운영자 구글 계정 하나(피싱·기기 분실·세션 탈취)가 전 회원 건강정보·신체 사진의
단일 실패점이다. 접근기록이 없어 유출 사실조차 사후에 입증할 수 없다 — 이건 기술 문제이면서
동시에 안전성 확보조치 위반이다.
...
| P1-7 | **규칙 단위 테스트 부재** + 규칙이 콘솔 수동 배포 | medium | `package.json` |
...
**수정**: 에뮬레이터 기반 규칙 테스트를 CI에 추가(§7-1의 시나리오를 그대로 테스트로),
규칙 배포를 `firebase deploy --only firestore:rules`로 CI화. 제안본 배포 시
`src/store.js:462`의 레거시 원형 복사가 거부될 수 있으니 마이그레이션 완료를 먼저 확인할 것.
