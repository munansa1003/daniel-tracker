...
describe("P0~P1 신규 섹션", () => {
  ...
  it("운동 구성 (주간) — 유형 % + 주간 총kcal + 운동일·휴식일 (조깅=유산소)", () => {
    expect(pkg).toContain("## 운동 구성 (주간)");
    // 열 6종: 유산소·근력·상체·하체·코어·기타 (근력은 2026-08 감사 R-05에서 추가 —
    // 워치 자동 수신 근력이 '기타'로 떨어져 상·하체 0%가 거짓으로 읽히던 것을 분리)
    expect(pkg).toContain("유산소  근력  상체  하체  코어  기타(%)");
    expect(pkg).toMatch(/07\/13주\s+100\s+0\s+0\s+0\s+0\s+0\s+\| 총\s+300kcal \| 운동 1일 · 휴식 4일/);
    expect(pkg).toContain("주간 총kcal"); // 헤더에도 병기
    // 근력이 없는 기간에는 각주를 붙이지 않는다(과거 출력 보존)
    expect(pkg).not.toContain("부위 정보가 없어");
  });
