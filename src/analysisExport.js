...
export function buildAnalysisPackage(state, { start, end }, todayStr, opts = {}) {
  ...
  const L = [];
  // 섹션 격리 — 한 섹션의 예외가 문서 전체를 비우지 못하게. 실패는 마커로 가시화.
  const safe = (name, fn) => {
    try { fn(); }
    catch (e) { L.push(`[생성 실패: ${name} — ${errMsg(e)}]`); L.push(""); }
  };
  let recordedCount = 0;
  ...
  try {
    ...
    // 기간 내 수집 — shape 오염(문자열 아닌 date, null start 등)은 여기서 걸러 하류 예외를 줄인다
    ...
    const weighs = bodyLog.filter((b) => b && typeof b.date === "string" && b.date >= start && b.date <= end && b.weight > 0);
    const events = (healthEvents || []).filter((ev) => ev && typeof ev.start === "string" && ev.start <= end && (ev.end || end) >= start);
    ...
    safe("설정 변경 이력", () => {
      const settingLines = [];
      const hist = [...(tdeeHistory || [])].filter((h) => h && typeof h.from === "string").sort((a, b) => a.from.localeCompare(b.from));
      let prevAdj = 0;
      for (const h of hist) {
    ...
    // 일별 요약 — 유효목표 병기 + 초과폭 + ≈ 플래그. 날짜 단위로 격리(오염된 하루가 뒤를 못 막게).
    safe("일별 요약", () => {
    ...
    safe("빈도 집계", () => {
      ...
      for (const ds of recorded) {
        for (const m of (Array.isArray(allDays[ds].meals) ? allDays[ds].meals : [])) {
          const f = foodFreq.get(m.n) || { n: 0, k: 0 };
          f.n++; f.k += (m.k || 0) * (m.serving || 1);
          foodFreq.set(m.n, f);
        }
        for (const e of (Array.isArray(allDays[ds].exercises) ? allDays[ds].exercises : [])) {
    ...
    // 최후 방어 — 설정 단계 자체가 죽어도 빈 문자열은 절대 반환하지 않는다
    L.push(`[생성 실패: 전체 — ${errMsg(e)}]`);
  }

  // 생성 정보 — 어떤 버전·조건으로 만들어진 문서인지 자기식별(빈 파일/구버전 논쟁 종결용)
  L.push("");
  L.push("---");
  L.push(`(생성정보: 내보내기 ${EXPORT_VERSION} · 기준일 ${todayStr} · 기록 ${recordedCount}일 · ${detail ? "정밀 상세본" : "코치 요약본"})`);
  return L.join("\n");
}
