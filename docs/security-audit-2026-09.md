...
| 유료 다중 사용자(경로 C) | ❌ 불가 | P0 11건 전부 + 법무 트랙 완료 전에는 불가 |
...
**다렌즈 반박 검증**(코드 사실관계 / 악용 가능성 / 영향·심각도)에 통과시켰다.

원시 발견 **130건** → 중복 병합·반박 검증 → 본문 항목 **59건**(P0 11 · P1 19 · P2 29).
...
**critical이 0건이라는 점을 강조해 둔다.** 타인 데이터 열람·인증 우회·비밀값 유출·RCE에
해당하는 발견이 없었다.

> **"차단 항목(blocker)"과 "심각도"는 다른 축이다.** P0에는 심각도가 medium인 것도 있다
> (P0-6·P0-7·P0-8·P0-9). 기술적 파괴력은 크지 않지만 **유료 다중 사용자로 가는 길을 구조적으로
> 막거나 법적으로 불가능하게 만드는** 항목들이다. 반대로 P1에도 지금 당장 고쳐야 할 것이 있다
> (P1-1·P1-12는 P0 묶음과 함께 처리할 것).

검증 과정에서 **판정이 바뀐 항목 44건**은 §9에 근거와 함께 남겼다. 1차 감사가 과대평가한
비용 추정·공격 시나리오가 다수였고, **내가 1차로 쓴 서술 중 사실 오류 3건도 여기 포함된다.**
...
| P0-11 | **프리뷰 배포가 자기 자신을 Origin 화이트리스트에 자동 등록** — 패치 이전 코드가 무기한 살아남음 | high | `security.js:17` | S |
...
### P0-11. 프리뷰 배포가 모든 수정을 우회 가능하게 만든다

**이 항목은 다른 P0를 전부 고쳐도 그 수정이 무력화될 수 있다는 뜻이라 따로 둔다.**

`api/_lib/security.js:16-17`:
...
**모든 배포가 자기 도메인을 Origin 화이트리스트에 자동 등록한다.** 즉 브랜치 푸시마다 생기는
프리뷰 URL이 전부 "정상 Origin을 가진 완전 동작 API"다. 그리고 프리뷰에 프로덕션 환경변수가
주입되어 있다면, 그 URL은 **같은 `ANTHROPIC_API_KEY`·KV·VAPID·`IMPORT_TOKEN`으로 동작한다.**

**문제는 이것이다.** `analyze-*`에 인증을 붙이고(P0-1), 입력 길이 상한을 넣고(P0-2),
`rateLimit`을 fail-closed로 바꿔도(P1-1) — **수정 이전 커밋의 프리뷰 URL은 그대로 살아 있다.**
공격자는 패치된 프로덕션이 아니라 패치 전 프리뷰를 때리면 된다. 배포한 수정이
"배포됐지만 우회 가능한" 상태가 된다.

이 저장소는 브랜치 푸시가 일상이고(CI가 `on: push`), 지금도 원격에 브랜치가 여럿 있다.

**수정** (전부 콘솔 설정에 가깝다)
1. **Vercel Deployment Protection을 프리뷰 전체에 켠다** — P0-6의 Pro 전환과 같은 작업 묶음.
2. `ANTHROPIC_API_KEY`·`IMPORT_TOKEN`·`VAPID_PRIVATE_KEY`·`INBODY_*`를 **Production 스코프 전용**으로
   재설정하고 Preview에는 두지 않는다. 미설정 시 503 fail-closed가 이미 동작한다.
3. `security.js:17`의 자동 등록을 `VERCEL_ENV === 'preview'`일 때로 한정하거나 아예 제거한다.

> 같은 함수의 `:9-11`도 함께 보라. **프로덕션에서도 `http://localhost:5173`·`:4173`이 무조건
> 허용 목록에 들어간다.** 공격자는 `PRODUCTION_ORIGIN` 값을 알 필요조차 없이
> `-H 'Origin: http://localhost:5173'` 한 줄로 통과한다. P0-1의 공격 난이도가 그만큼 더 낮다.
...
| P1-15 | **지출 한도·사용량 계측이 어디에도 없음** — 사고를 청구서 전까지 감지 못함 | medium | 운영 설정 전반 |
| P1-16 | AI 응답 수치 검증에 **NaN·음수가 통과** | medium | `analyze-food.js:109,125` |
| P1-17 | 오류 응답의 `raw`가 **모델 출력 회수 채널**이 된다 | medium | `analyze-food.js:104,120,126` |
| P1-18 | `/export/diag` + **공개 저장소의 이 문서** = 살아 있는 취약점 지도 | medium | `export-view.js:43` |
| P1-19 | 서버 쓰기 실패가 **UI에 전혀 드러나지 않음** — 대기열을 읽는 화면이 0곳 | medium | `store.js:320` |
| P1-20 | 백업에 **진행 사진이 빠져 있음** — 열람·이관 요구에 응할 수단 없음 | medium | `backup.js:38-52` |
...
- **KV blast radius**: 하나의 Upstash 인스턴스·하나의 전권 토큰에 rate limit·공유 스냅샷(건강 데이터)·
  수신 사서함·푸시 구독이 전부 들어 있다.

  > **다만 "인스턴스를 분리하자"는 수정안은 기각됐다.** 이 발견이 상정하는 유일한 전제는
  > "Vercel env에서 `KV_REST_API_TOKEN`이 샌다"인데, **그 전제가 성립하는 순간 같은 env의
  > `ANTHROPIC_API_KEY`·`VAPID_PRIVATE_KEY`·`IMPORT_TOKEN`·`INBODY_LOGIN_PW`도 함께 샌다.**
  > 인스턴스를 둘로 쪼개도 토큰 둘이 같은 env에 있으므로 줄어드는 것이 없다.
  >
  > **실제로 효과가 있는 것은 암호화다.** 공유 스냅샷을 서버 키로 암호화해 두면(P1-4)
  > KV 토큰이 새도 건강 데이터 본문은 암호문이다. 그리고 **eviction 정책을 `noeviction`으로
  > 확인**해야 한다 — `allkeys-lru`면 메모리 압박 시 인바디 계정 잠금 방어 키
  > (`import:body-cloud-authblock` 등)가 먼저 사라져 3회 재시도가 되살아난다.
...
### P1-15. 지출 한도·사용량 계측이 어디에도 없다

**§2의 가용성 공격 넷(P0-1·P0-3·P0-4·P1-12)이 전부 "조용히" 일어난다는 점이 이 항목의 핵심이다.**
Anthropic·Vercel·Firebase·Upstash 어느 쪽에도 지출 한도나 사용량 알림이 설정돼 있다는 근거가
코드·문서에 없다. 크론이 잘려 뒤쪽 사용자가 누락돼도 로그에 원인이 남지 않고(`cron-reminders.js:128`의
결과 JSON이 반환되지 않는다), KV가 차오르는 것도 500이 터지기 전까지 보이지 않는다.

**즉 탐지 계층이 통째로 없다.** 이건 다른 발견들의 파생이 아니라 독립된 결함이다. P0를 전부
고쳐도 남는다 — 다음 사고는 다른 모양으로 올 것이고, 그때도 청구서나 사용자 문의가 첫 신호가 된다.

**수정**(전부 콘솔 설정, 코드 0줄)
1. **Anthropic**: 앱 전용 워크스페이스 + 지출 한도 + 사용량 알림.
2. **Firebase**: 예산 알림(Blaze인 경우), Firestore 읽기 급증 알림.
3. **Upstash**: 저장 용량·명령 수 알림, eviction 정책을 `noeviction`으로 확인.
4. **Vercel**: Spend Management 알림, 함수 호출·실행 시간 대시보드 확인.
5. 크론 결과(`checked`/`sent`/`cleaned`)를 KV에 남겨 며칠치를 설정 화면에서 볼 수 있게 한다
   — 지금은 함수가 잘리면 그 숫자 자체가 사라진다.

### P1-16. AI 응답 수치 검증에 NaN·음수가 통과한다

`api/analyze-food.js:109`의 필터는 `f.n && typeof f.p === "number"`다. 그런데
**`typeof NaN === "number"`는 참**이고, 음수도 막지 않는다. `:125`의 텍스트 경로도 같다.

그 값이 그대로 흘러간다.

```js
const calcK = f.p * 4 + f.c * 4 + f.f * 9;   // NaN이면 calcK도 NaN
if (!f.k || Math.abs(f.k - calcK) > calcK * 0.3) f.k = calcK;  // !NaN === true → f.k = NaN
```

즉 `NaN` 또는 음수 영양소가 **클라이언트 렌더와 Firestore 저장까지 도달한다.** 저장되면
그날의 합계·목표 판정·주간 성적표·적응형 TDEE 계산이 전부 오염된다.

> 운동 경로는 막혀 있다 — `analyze-exercise.js:89`가 `ex.m < 1.0 || ex.m > 23.0`으로 범위를
> 검사한다. **구멍은 음식 경로뿐이다.**

**수정** (1줄)
```js
const okNum = (v, lo, hi) => Number.isFinite(v) && v >= lo && v <= hi;
const validated = foods.filter(f =>
  typeof f.n === "string" && f.n.length <= 60 &&
  okNum(f.p, 0, 500) && okNum(f.c, 0, 1000) && okNum(f.f, 0, 500)
);
```

### P1-17. 오류 응답의 `raw`가 모델 출력 회수 채널이다

`analyze-food.js:104`·`:120`·`:126`은 JSON 파싱에 실패하면
`{ error: "AI 응답 파싱 실패", raw: text }`를 반환한다. **`text`는 Claude 응답 본문 전체다.**
`analyze-exercise.js:74`·`:78`·`:94`, `analyze-body.js:66`도 같다. 세 파일 모두 마지막 `catch`가
`message: e.message`를 함께 돌려준다.

**앞서 "프롬프트 인젝션은 실용적 범용 프록시가 못 된다"고 판정한 근거를 재검토해야 한다.**
출력이 300~800토큰으로 잘려 실용성이 낮은 것은 맞지만, **회수 자체는 된다.**
`raw`가 정확히 그 채널이다. 파싱 정규식이 `/\{[^}]+\}/`라 산문 응답이면 반드시 실패하므로,
공격자는 오히려 **JSON이 아닌 답을 유도하는 편이 쉽다.**

남는 실질 위험은 두 가지다. 운영자 Anthropic 키에 귀속되는 **목적 외 생성물이 실제로 반출**되는 것
(이용정책 위반 시 계정 단위 조치 대상), 그리고 서버 예외 메시지가 외부로 나가는 것이다.

**수정**(5줄 미만, P0-1보다 먼저 가능): `raw: text`와 `message: e.message`를 제거하고
서버 로그로만 남긴다(`console.error`는 이미 있다). 파싱을 정규식 대신 구조화 출력으로 바꾸면
실패 경로 자체가 줄어든다.

### P1-18. 진단 엔드포인트 + 공개 저장소 = 살아 있는 취약점 지도

**이 항목은 이 문서 자체가 만든 위험이라 특별히 적어 둔다.**

`api/export-view.js:43-53`은 인증 없이 다음을 200으로 반환한다:
`commit`(배포 커밋 SHA 앞 7자) · `branch` · `vercelEnv` · `tokenConfigured` · `tokenLength` · `shareEnabled`.
`vercel.json:13`이 `/export/diag`를 고정 경로로 노출한다.

그리고 **이 저장소는 공개(public)다.** 이 문서는 미수정 P0를 파일·행 단위로 나열하고 있다.

두 가지를 합치면 이렇게 된다.

```
curl https://<도메인>/export/diag      →  배포 커밋이 abc1234임을 알아낸다
→ 공개 저장소에서 abc1234 시점의 docs/security-audit-2026-09.md를 읽는다
→ "이 커밋에 어떤 P0가 아직 안 고쳐졌는지"를 문서로 확인한다
```

정찰 비용이 사실상 0이 되고, 패치 배포 여부까지 실시간으로 추적된다.

**수정**
1. `/export/diag`에 토큰 대조를 붙이고(없으면 404), `commit`·`branch`·`tokenLength`를 제거한다.
   `vercel.json:13`의 rewrite도 삭제해 고정 경로 노출을 없앤다.
2. **P0 수정이 끝나기 전까지는 이 문서와 `docs/system-audit-2026-08.md`를 비공개로 옮기는 것을
   권고한다.** 또는 저장소 자체를 비공개로 전환한다(§3 P1-10과 같은 작업).
   수정이 끝난 뒤 공개하면 이 문제는 사라진다.

### P1-19. 서버 쓰기 실패가 화면에 전혀 드러나지 않는다

`src/store.js:320`의 `set()`은 실패 시 조용히 `return false`한다. **호출부가 그 값을 보지 않는다.**
그리고 더 중요한 것: 실패한 키는 `src/syncQueue.js`의 대기열에 쌓이는데,
**`getPending`을 읽는 코드가 `store.js`와 `syncQueue.js` 내부뿐이다. 대기열을 보여주는 화면이 0곳이다**(grep 확인).

영구적 실패(멤버십 회수 후 `permission-denied`, App Check 실패, `bodylog`·`day` 문서가
Firestore 1MiB 한도 초과, 쿼터 소진)가 나도 **화면은 정상이다.** 사용자는 로컬 미러만 보며
저장됐다고 믿고, 기기를 바꾸거나 브라우저 데이터를 지우는 순간 그 이후 전부를 잃는다.

보안 결함이라기보다 **유료 서비스에서 가장 비싼 유형의 사고**(환불·분쟁)다.
§3 P1-2(로그아웃 purge)와도 직결된다 — 대기열이 비었는지 모르면 purge를 안전하게 할 수 없다.

**수정**: `flushPendingSync` 후 `getPending(uid).length > 0`이면 헤더에 "동기화 대기 N건" 배지를
띄우고 탭하면 재시도한다. 같은 키가 연속 실패하면 원인 코드(`permission-denied` /
`resource-exhausted` / `invalid-argument`)를 구분해 안내한다.

### P1-20. 백업에 진행 사진이 빠져 있다

`src/backup.js:38-52`의 `buildBackup`이 담는 것은 `days`·`bodylog`·`goals`·`customFoods`·
`customExercises`·`bodyDrafts`·`profile`뿐이다. **진행 사진은 없다**(grep 확인).
사진은 별도 서브컬렉션 `users/{uid}/data/photos/items/{id}`에 있어 `store.getAllData()`의
직속 문서 순회에도 잡히지 않는다.

**이 앱이 보관하는 가장 민감한 자료인데, 사용자가 자기 데이터를 통째로 가져가는 경로에서
정확히 그것만 빠진다.** 열람·이관 요구(§2 P0-5)에 응할 수단이 없고, 탈퇴 시 "전부 지웠다"를
증명할 목록도 만들 수 없다. 기기 교체 시 조용히 사라지는 유실 경로이기도 하다.

**수정**: 계정 데이터 수명주기를 함수 둘로 묶는다.
`exportAll(uid)` = data 컬렉션 + `photos/items` 서브컬렉션(+ KV 보유분 목록),
`deleteAll(uid)` = 같은 열거로 삭제. 사진이 문서 내 base64라 열거만 하면 둘 다 클라이언트
SDK로 된다. §2 P0-5의 계정 삭제와 같은 작업이다.
...
| `store.set`의 `localStorage.setItem`이 **try/catch 없이** 큐 등록·Firestore 쓰기보다 앞에 있음 | `store.js:313` | 용량 초과 시 기록이 **통째로 유실**된다(Firestore 쓰기까지 도달 못 함). 같은 파일의 다른 `setItem`은 전부 `try/catch`로 감싸여 있어 이 한 곳만 예외다 |
| `.gitignore`가 3줄뿐 (`node_modules`·`dist`·`.env`) | `.gitignore` | `.env.local`·`.env.*.local`·`*.pem`·`*.key`·`.vercel` 추가. 공개 저장소라 실수 1회의 비용이 크다 |
| 공유 링크 **조회 카운터를 아무도 읽지 않음** | `share-store.js:65-72` | `share:hits:{token}`을 INCR만 하고 읽는 코드가 0곳이다. 링크 유출을 판단할 유일한 신호를 수집하면서 버린다. 링크 카드에 "조회 N회" 표시 + 과다 조회 시 폐기 유도 |
| 공용 음식/운동 DB가 **캐시 폴백 + 전체 덮어쓰기** | `store.js:121-165` | 읽기 실패 시 옛 localStorage 목록을 반환하고 그 위에 `setDoc`으로 문서 전체를 덮는다. 삭제는 인덱스 기반이라 어긋나면 다른 항목이 지워진다. `arrayUnion`/`arrayRemove` 또는 트랜잭션으로, 삭제는 이름·id 기준으로 |
| **로그인 수단이 Google 하나** — 계정 복구·이관 경로 없음 | `auth.js:34-47` | 구글 계정 정지·회수 시 유료 고객이 자기 기록에 영영 접근 못 한다. 운영자도 도울 수단이 없다(`members`는 update/delete 전면 차단). 이메일 링크나 Apple 로그인을 2차 수단으로 추가하거나, 최소한 약관에 "복구 불가"를 명시하고 백업을 강하게 유도 |
...
### 7-4. 완전성 비평이 남긴 질문 (아무 관점도 묻지 않은 것들)

- **프리뷰 배포에 프로덕션 env가 주입되는가? Deployment Protection은 켜져 있는가?**
  살아 있는 프리뷰 URL이 몇 개이고 가장 오래된 것은 어느 커밋인가 — §2 P0-11의 심각도가 여기 달렸다.
- `bodylog`·`body-drafts`·`day:{date}`가 각각 **단일 Firestore 문서**인데, 자동 수신이 매일 쌓이는
  다년 사용자에서 **1MiB 한도에 언제 닿는가?** 한도 초과 시 `store.set`은 조용히 `false`를
  반환하고 화면은 아무것도 표시하지 않는다(§3 P1-19). 도달 시점 산정과 분할 설계가 있는가?
- **Upstash의 백업·스냅샷 보존 기간이 공유 TTL(최대 7일)보다 긴가?** 길다면
  "즉시 폐기됩니다"라는 UI 문구가 사실과 어긋난다.
- Firebase Identity Platform의 `accounts:lookup` **쿼터와 지연 SLO는 얼마인가?**
  모든 인증 API의 응답 시간과 실패율이 이 외부 왕복에 1:1로 묶여 있고 캐시도 타임아웃도 없다.
- **구글 계정이 정지·삭제된 유료 사용자의 데이터 접근을 어떻게 복구하는가?**
  로그인 수단이 하나이고 데이터가 uid에 결박돼 있으며 `members`는 update/delete가 전면 차단이다.
- **운영자 부재(사고·계정 잠금·장기 휴면) 시 유료 계약 이행과 데이터 승계는 누가 하는가?**
  현재 운영자 1인이 유일한 관리 신원이자 유일한 전량 read 권한자다. 사업 연속성은 유료화의 전제다.
- **이 문서와 `docs/system-audit-2026-08.md`를 공개 저장소에 계속 둘 것인가?** (§3 P1-18)
...
둘 중 하나라도 나쁘면 아래 순서가 바뀐다. 동시에 **P1-15(지출 한도·사용량 알림 4곳)**을
설정한다 — 코드 0줄이고, 이후 모든 사고의 탐지 계층이 된다.
...
P0-11  Deployment Protection + 시크릿을 Production 스코프로 + VERCEL_URL  (S)  ← P0-6과 같은 묶음
P0-7   IMPORT_*·INBODY_* env 비우기(기능 off) 또는 재설계 결정           (S/L)
P1-2   로그아웃 purge                                                   (S)
P1-3   vercel.json 보안 헤더                                            (S)
P1-16  AI 응답 NaN·음수 필터 1줄                                        (S)
P1-17  raw·e.message 제거 (P0-1보다 먼저 가능)                          (S)
P1-18  /export/diag 잠그기 + 감사 문서 비공개 전환                      (S)
```
...
P1-8/11  클라이언트 재검증 · KV 위생(암호화 + noeviction 확인)
P1-19    동기화 대기 배지 + 실패 원인 안내
P1-20    exportAll/deleteAll 한 쌍 (계정 삭제와 같은 작업)
...
- [ ] P0 **11건** 전부 해결 또는 해당 기능 비활성화
- [ ] 프리뷰 배포가 프로덕션 시크릿을 갖지 않음 + Deployment Protection 활성 (P0-11)
...
| (신규) "지출 한도·계측 부재는 다른 발견의 파생" | 아니다. 독립 결함이다. P0를 전부 고쳐도 탐지 계층은 여전히 없고, 다음 사고의 첫 신호가 청구서나 사용자 문의가 된다 → P1-15 |
| (신규) AI 응답 검증이 `typeof === "number"`뿐 | `typeof NaN === "number"`가 참이라 NaN·음수가 통과해 Firestore 저장까지 도달한다. 운동 경로는 범위 검사가 있어 막히고 **음식 경로만 구멍**이다 → P1-16 |
| "KV 인스턴스를 분리하면 사고 반경이 준다" | 수정안이 위협을 해결하지 못한다. 전제인 "Vercel env에서 KV 토큰이 샌다"가 성립하면 같은 env의 Anthropic 키·VAPID 개인키·IMPORT_TOKEN·인바디 비밀번호도 함께 샌다. 효과가 있는 것은 **암호화**와 eviction 정책 확인이다 |
| (신규) "프롬프트 인젝션은 출력을 회수할 수 없어 무해" | **틀렸다.** 파싱 실패 시 `raw: text`가 모델 응답 전문을 돌려준다. 정규식이 `/\{[^}]+\}/`라 산문 응답이면 반드시 실패하므로 오히려 JSON이 아닌 답을 유도하는 편이 쉽다 → P1-17 |
| (신규) "P0를 고치면 그 경로는 닫힌다" | **프리뷰 배포가 자기 도메인을 Origin 화이트리스트에 자동 등록**하므로, 패치 이전 커밋의 프리뷰 URL이 같은 시크릿으로 계속 동작한다. 수정이 "배포됐지만 우회 가능한" 상태가 된다 → P0-11 |
| (신규) "공격자는 PRODUCTION_ORIGIN을 알아야 한다" | 아니다. 프로덕션에서도 `http://localhost:5173`이 무조건 허용 목록에 있어 그 값 하나로 통과한다 |
| (신규) "이 감사 문서는 공개돼도 무방하다" | `/export/diag`가 배포 커밋을 알려주므로, 공개 저장소의 이 문서는 "지금 배포본에 살아 있는 취약점 목록"으로 그대로 쓰인다 → P1-18 |
