...
export default async function handler(req, res) {
  ...
  const { uid, idToken, action, keys } = req.body || {};
  ...
  try {
    ...
    if (action === "ack") {
      ...
      const safe = keys.filter((k) => typeof k === "string" && k.length > 0 && k.length <= 300);
      if (!safe.length) return res.status(400).json({ error: "keys required" });
      const removed = await kv("HDEL", inboxKey, ...safe);
