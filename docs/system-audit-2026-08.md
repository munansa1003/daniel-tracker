...
승인 13건의 처방을 한 줄씩 되짚어 "적어둔 것보다 약하게 하지 않았나"를 확인했다.

| ID | 처방 | 이행 |
|---|---|---|
| R-01 | `SET NX EX` + 반환 `null`이면 throttled · `failKey`도 `INCR` | ✅ 둘 다 |
| R-02 | 집합 문서는 원격과 병합해 전송 | ✅ + 처방에 없던 **삭제 흔적**을 더해 부활 사고를 막음 |
| R-03 | ①시각 노출 + 저녁 경고 ②분석 계약 전달 ③되돌림 시 혼재 문서화 | ✅ ①② · **③이 운영 문서에 없던 것을 이번에 보완**(`inbody-setup.md`) |
| R-04 | ①체성분 시각·자동/수동 ②운동 ⌚ ③측정 규칙 줄 | ✅ 셋 다 |
| R-05 | ①근력 열 분리 ②각주 | ✅ 둘 다 |
| R-06 | 크론이 수신 로그 최신 `at`을 직접 읽어 N일 침묵 시 푸시 | ✅ + 토글(`reminders.sync`)까지 |
| R-07 | `buildBackup`에서 제외 + 복원 시 미복원 | ✅ — 복원은 "미복원"보다 나은 **현재 링크 이월**로 |
| R-08 | 쓰기 전 `revoked` 재확인 **또는** 별도 카운터 키 | ✅ 경합이 아예 없는 **카운터 키** 쪽 |
| R-09 | ①총 예산 **또는** ②요청 타임아웃 인하 | ✅ **둘 다** |
| R-10 | 그 날짜 기준 고정 + 골든셋·297일 diff 필수 | ✅ 둘 다 |
| R-11 | 유입 4경로·저장소·소비처·소급 전파·비공식 API 리스크 | ✅ — **소비처가 빠졌던 것을 보완** |
| R-39 | `rateLimit`을 토큰 대조 앞으로 | ✅ 두 검문소 모두 |
| R-41 | 훅·CI를 `eslint src api`로 | ✅ + `CLAUDE.md` 정의 동기화 |

**약화된 것 0건.** 처방보다 강하게 간 것 3건(R-02 흔적 · R-08 카운터 키 · R-09 둘 다),
대조 과정에서 **빠졌던 것을 채운 것 2건**(R-03③ · R-11 소비처).

---

# Phase 7 — 백로그 이행 (승인: "P0+P1 전부 진행하고, R-41도 게이트 확장도 진행해줘" → "PR 머지해줘. 그리고 나머지 작업도 진행하자")

PR [#101](https://github.com/munansa1003/daniel-tracker/pull/101) 병합 후 백로그 37건을 5개 배치로 처리했다.
아래 표가 **레지스터 50건 전체의 최종 처분**이다. "미이행"에는 이유를 적었다 — 조용히 빠뜨린 것은 없다.

## 7.1 이번에 수정한 것 (25건)

| 배치 | ID | 한 줄 요약 |
|---|---|---|
| 1 데이터 정합 | R-14 | 같은 날 같은 `sampleTs`가 HAE·인바디 양쪽에서 오면 나중 것이 덮던 것 → 키에 `source` 추가 |
| | R-16 | 형식이 깨진 유입 항목이 ack 안 돼 사서함에 영구 잔류 → 파싱 실패도 ack |
| | R-27 | `isEveningSample`이 `getHours()`라 UTC 서버에서 아침이 저녁으로 판정 → 고정 +09:00 + `getUTCHours()` |
| | R-28 | `aggregateDay`의 NaN 전파로 합계 전체가 사라지던 것 → 항목 단위 가드 |
| | R-42 | 운동 시간을 고치면 워치 **실측** kcal이 MET 추정치로 교체 → 실측은 시간 비율로만 조정 |
| | R-43 | 백업에 프로필(키·나이)이 없어 복원 시 목표가 조용히 달라짐 → 선택 필드로 포함 |
| 2 보안·방어 | **R-40** | **`CRON_SECRET` 미설정 시 검문을 통째로 건너뛰어 누구나 푸시 발송 가능** → fail-closed(503) |
| | R-23 | 재발급해도 이전 공유 링크가 TTL까지 생존 → 발급이 곧 이전 링크 폐기 |
| | R-22 | 로그인 없이 열리는 링크에 컨디션 자유기술이 실리는데 경고가 없음 → 발급 전후 문구 모두 보강 |
| | R-31 | `.env.example`에 실제 사용 env 9종 누락 → 전부 추가 + 대조 스크립트로 확인 |
| | R-25 | 인바디 자격증명의 사고 반경(계정 전체) 미문서화 → `.env.example`에 명시 |
| 3 관측·UX | R-12 | 운동 카드 "지금 확인"에 진행 중 가드 없음(체성분 카드엔 있음) → 동일 가드 |
| | R-20 | 스트릭이 휴식일 도장을 무시해 계획대로 쉰 날이 "끊김" → 잇되 세지는 않는다 |
| | R-21 | 미확정 초안이 쌓여도 "체중 재세요" 독촉 지속 → `lastMeasuredDate`(확정분 ∪ 초안) |
| | R-26 | AI 캐시 정리가 Firestore 사본을 남겨 다음 동기화에 부활 → `store.delete` |
| | R-30 | 클라우드 pull "최근 5건" 고정이라 공백이 길면 유실 → 마지막 **성공** 기준 동적 산정 |
| | R-44 | 두 번째 기기가 크론 스냅샷 날짜를 과거로 되돌릴 수 있음 → 필드별 단조 병합 |
| 4 구조 부채 | R-33 | **CLAUDE.md 위반** — 체지방량 인라인 재구현 → `fatMassOf` 단일 출처 |
| | R-34 | 주간 등급 산식 2벌 복붙 → `weekGrade` 추출 + 경계값 테스트 |
| | R-47 | `cron-reminders`에 `maxDuration` 없음(잘리면 뒤쪽 uid 조용히 누락) → 60초 |
| | R-49 | 운영자 이메일 이중 정의에 정합성 검사 없음 → 두 파일을 직접 읽어 대조하는 테스트 |
| | R-50 | `StatsTab` 폴백이 목표 체중을 현재 체중 자리에 넣고 키·나이 하드코딩 → 실측·프로필 사용 |
| 5 통합·문서 | R-35 | 전 구간 관통 테스트 **0개** → 5케이스 신설 |
| | R-46 | 새 기기에서 백업 리마인더 15일 침묵 → 기록으로 계정 나이 판정 |
| | R-48 | CSV에 출처·체성분 점수 없음 → 열 추가 |

`R-32`의 실제 중복 키(`bodyCloudEnabled` 2회)도 제거했다 — 규칙 추가와 별개인 단순 결함이라서다.

## 7.1-b 문서화로 처리한 것 (3건) — 고칠 것이 아니라 알아야 할 것

`ARCHITECTURE.md` §6 지뢰 18~20에 넣었다. 셋 다 "동작이 틀렸다"가 아니라
"이렇게 동작한다는 걸 모르면 나중에 데이터가 샌다"에 해당한다.

| ID | 내용 |
|---|---|
| R-15 | 카드에서 **손확정한 날은 영구 잠금**이다(`draftToRecord`가 `auto`를 안 붙임). 클라우드가 복구돼도 그 날짜는 자동 갱신 대상이 아니다 — B채널 폴백으로 며칠을 손확정했다면 그만큼이 영영 수동 관리 대상 |
| R-17 | 귀속일 기준이 경로마다 다르다: 운동=시작 시각의 로컬 날짜 · 체성분=`IMPORT_TZ_OFFSET` · 크론="오늘"이 KST 고정 · 앱="오늘"이 기기 현지. 국내에서는 넷이 일치하나 해외(비-JST)에서는 갈린다 |
| R-45 | `IMPORT_TZ_OFFSET`을 바꾸면 같은 측정의 `sampleTs`(epoch)가 달라져 사서함 키가 새로 생긴다 — 확정된 날이 새 초안으로 되돌아올 수 있다(`date`·`hour`는 불변이라 눈에 안 띈다). 되돌아온 초안은 폐기하면 tombstone이 기억한다(R-02) |

## 7.2 기각 — 고치지 않는 것이 맞다고 판단한 것 (1건)

**R-24 방어 키 TTL.** 처방은 "무한 증가하는 방어 키에 TTL"이었다. 실제로 무한 증가하는 것은
`import:seen:*`·`import:body-seen:*` 두 종뿐이고(나머지는 `LTRIM`·`EX`·uid당 1개로 이미 유한),
**그 도장이 곧 insert-only 중복 차단의 근거다.** `mergeImports`는 항목이 day 기록에 남아 있을 때만
멱등하므로(`importMerge.js:67`), 사용자가 지운 운동은 도장이 사라지면 재유입 시 되살아난다.
증가량은 연 2,000개 남짓(10년에 수 MB)이다 — 검증된 불변식을 깎아 되사기에는 값이 맞지 않는다.
감사 프롬프트의 "검증된 규칙 임의 완화 금지"에도 정면으로 걸린다.

작업 중 실제로 180일 TTL을 넣었다가 이 판단으로 되돌렸다. 되돌림도 기록으로 남긴다.

## 7.3 미이행 — 사용자 지시가 필요한 것 (2건)

| ID | 왜 지시가 필요한가 |
|---|---|
| R-32 | ESLint에 `no-dupe-keys` 추가. `CLAUDE.md`가 "규칙 변경은 별도 단계에서 진행"으로 못박고 있다. 중복 키 **실물**은 이번에 제거했으나 규칙 자체는 손대지 않았다 |
| R-36 | `firestore.rules` 필드 검증 강화. 규칙 배포는 되돌리기 어렵고, 잘못 좁히면 정상 쓰기가 조용히 막힌다 — 배포 창을 사용자가 잡아야 한다 |

## 7.4 미이행 — 판단해 남긴 것 (5건)

| ID | 남긴 이유 |
|---|---|
| R-18 | 같은 유산소를 수동으로 또 넣었을 때의 **이중 계상 경고**. 처방이 "±15분·동일 종목이면 경고 표시"인데, 문턱을 정하는 순간 오탐(진짜로 두 번 뛴 날)이 생긴다 — 사용자가 어느 쪽 실수를 더 싫어하는지 모르는 채로 정할 값이 아니다 |
| R-19 | 자동 유입분 **일괄 삭제 UI**. 감사 범위(구조 결함 시정)를 넘는 기능 추가다. 삭제 흔적(R-02)·출처 표기(R-04·R-48)가 갖춰져 설계 전제는 준비됐으므로, 기능으로 따로 잡는 편이 맞다 |
| R-29 | `adaptiveOn` 토글 OFF를 이력에 기록. **과거 판정이 바뀌는 변경**이라 골든셋·297일 회귀를 다시 깔고 사용자와 의도를 맞춘 뒤 하는 것이 옳다 |
| R-37 | 사서함 적재 상한. 6개월+ 미개봉이라는 전제가 현실성이 낮고, 지금 넣으면 상한 자체가 새 실패 모드가 된다 |
| R-38 | 인바디 JWT(24h) 캐시. 로그인 횟수를 줄이는 이점은 있으나, **인바디 계정 접근 토큰을 KV에 한 벌 더 두는 일**이다(R-25에서 사고 반경을 좁히자고 문서화한 직후다). 현재 상한은 하루 ~48회로 앱 수준을 넘지 않는다 |

## 7.5 운영자 조치 필요 (배포 전 반드시)

**Vercel 환경변수에 `CRON_SECRET`을 설정할 것.** R-40으로 이 값이 **필수**가 됐다 —
없으면 밤 8시 예약 푸시가 503으로 멈춘다. 값이 없을 때 열어두는 쪽(누구나 구독자 전원에게
푸시 발송)이 더 위험하다는 판단에 따른 의도적 fail-closed다.
생성: `openssl rand -hex 32` · Vercel Cron이 `Authorization: Bearer <secret>`를 자동 첨부하므로
값만 넣으면 그대로 동작한다. 증상 확인은 Vercel 로그의 `503` + `CRON_SECRET 미설정`.
...
| 테스트 | 368 | **479** (+111) |
| 테스트 파일 | 34 | 40 |
| 정적 검사 범위 | `eslint src` | `eslint src api` |
| 전 구간 통합 테스트 | 0 | 5 |
| 레지스터 50건 처분 | — | 아래 표 (합계 검증 완료 — 중복 0 · 누락 0) |

| 처분 | 건수 | ID |
|---|---|---|
| PR #101에서 수정 | 14 | R-01~R-11 · R-13 · R-39 · R-41 |
| 백로그에서 수정 | 25 | R-12 · R-14 · R-16 · R-20~R-23 · R-25~R-28 · R-30 · R-31 · R-33~R-35 · R-40 · R-42~R-44 · R-46~R-50 |
| 문서화로 처리 | 3 | R-15 · R-17 · R-45 |
| 기각(고치지 않는 것이 맞음) | 1 | R-24 |
| 사용자 지시 대기 | 2 | R-32 · R-36 |
| 판단해 보류 | 5 | R-18 · R-19 · R-29 · R-37 · R-38 |
| **합계** | **50** | — |

레지스터와 별개로 **정상 확인 30건**과 **반증된 지적 3건**은 Phase 3에 그대로 있다.

---

# Phase 8 — 잔여 백로그 (PR #102 병합 후, "나머지 작업도 진행하자")

PR [#102](https://github.com/munansa1003/daniel-tracker/pull/102) 병합 후, 남아 있던 7건 중 4건을 처리했다.

## 8.1 처리한 것 (4건)

| ID | 처리 |
|---|---|
| **R-32** | ESLint에 `no-dupe-keys` 추가(사용자 지시). **추가 즉시 실물 2건이 걸렸다** — `App.jsx`의 `bodyCloudEnabled`(지난 배치에서 제거)와 `StatsTab.jsx`의 `padding`. 후자는 뒤엣것이 앞엣것을 덮고 있었고 두 값이 우연히 같아 화면상 차이가 없었다. 값이 달랐다면 "에러 없이 숫자만 틀린다"(지뢰 2번)로 나타났을 자리다. `CLAUDE.md`의 "최소 규칙 3개"도 4개로 동기화 |
| **R-37** | ack의 500건 상한을 **없애고 200건씩 나눠 지운다.** 사서함 *적재*에는 상한이 없어 오래 안 열면 ack가 영영 실패하고 → 사서함이 안 비고 → 다음 pull에도 같은 수가 내려오는 교착이 됐다. 오류 메시지도 `keys required`라 "키가 없다"로 읽혀 원인 추적이 엉뚱한 곳을 가리켰다. **상한을 올리는 대신 나눠서** 지운 이유: 상한은 그 자체가 새 실패 모드다 |
| **R-19** | 자동 수신분 기간 일괄 삭제(설정 → 데이터). 미리보기 → 확인(같은 건수 재고지) → **안전본 자동 다운로드** → 삭제 순서. 지우는 대상은 자동 유입분만(`source:"watch"` / `auto:true` / `source:"import"` / 초안 전부), 손입력은 어떤 경우에도 건드리지 않는다. 삭제 흔적(tombstone)까지 남겨 재유입으로 되살아나지 않게 했다. 계산은 순수 모듈(`bulkDelete.js`)로 분리 — 파괴적 동작을 순수 함수로 떼어야 "무엇이 지워지는가"를 지우기 전에 보여줄 수 있다 |
| **R-18** | 수동 입력이 자동 수신분과 겹치면 경고. **막지 않고 알린다** — 진짜로 두 번 운동한 날이 있으므로 차단하면 정상 기록을 막는다. 겹침 기준은 같은 날·`source:"watch"`·시간대 **±1시간**·종목명 상호 포함. 레지스터의 처방은 ±15분이었으나 **저장된 시간 해상도가 "시(hour)"뿐이라 분 단위 비교가 불가능하다** — 이 데이터로 표현 가능한 가장 좁은 창을 썼고, 그 사실을 여기 남긴다 |

R-19를 넣으면서 **설정 → 데이터 화면이 기존 스모크가 한 번도 열지 않던 곳**임을 발견해
렌더 경로 테스트도 함께 추가했다. 작성 중 `"*"` 셀렉터가 `onClick` 없는 바깥 래퍼를 먼저 잡아
클릭이 조용히 무시되는 것을 발견해 `button`으로 좁혔다 — 그대로 뒀으면 화면을 열지도 못한 채
통과하는 테스트가 될 뻔했다(테스트가 스스로를 검증해야 하는 이유).

## 8.2 남은 3건

| ID | 상태 | 왜 |
|---|---|---|
| **R-36** | 사용자 판단 필요 | `firestore.rules` 필드 검증 강화. 규칙 파일은 쓸 수 있으나 **배포는 사용자만 할 수 있고**, 잘못 좁히면 정상 쓰기가 조용히 막힌다(증상이 "가끔 저장이 안 됨"으로 나타나 원인 추적이 어렵다). 배포 창을 잡을 수 있을 때 함께 진행하는 것이 맞다 |
| **R-29** | ✅ 수정 완료 (§8.4) | 실데이터로 영향을 먼저 측정한 뒤 사용자 승인을 받아 진행 |
| **R-38** | 판단해 보류 유지 | 인바디 JWT(24h) 캐시. 로그인 횟수를 줄이는 이점은 있으나 **인바디 계정 접근 토큰을 KV에 한 벌 더 두는 일**이다 — R-25에서 사고 반경을 좁히자고 문서화한 직후에 반대 방향으로 가는 변경이다. 현재 상한은 30분 스로틀 기준 하루 ~48회로 실제 앱 사용 수준을 넘지 않는다 |
...
|---|---|---|
| PR #101에서 수정 | 14 | R-01~R-11 · R-13 · R-39 · R-41 |
| PR #102에서 수정 | 25 | R-12 · R-14 · R-16 · R-20~R-23 · R-25~R-28 · R-30 · R-31 · R-33~R-35 · R-40 · R-42~R-44 · R-46~R-50 |
| 이번 배치에서 수정 | 5 | R-18 · R-19 · R-29 · R-32 · R-37 |
| 문서화로 처리 | 3 | R-15 · R-17 · R-45 |
| 기각 | 1 | R-24 |
| 사용자 판단 필요 | 2 | R-36 · R-38 |
| **합계** | **50** | 중복 0 · 누락 0 (스크립트 대조) |

**수정 44건 · 문서화 3건 · 기각 1건 · 판단 대기 2건.**

테스트 368 → 501 (+133). 정적 검사는 `eslint src api` + 규칙 4개.


## 8.4 R-29 — 측정하고 고쳤다

처방을 바로 적용하지 않고 **실제 영향을 먼저 쟀다.** 과거 판정이 눈에 보이게 달라질 수 있는
변경이라, 숫자 없이 결정할 일이 아니었다.

### 측정 (사용자 실데이터, 백업 286일)

```
adaptiveOn  : false
tdeeHistory : [{ from: "2026-07-03", adjust: +68 }]
목표 kcal 변동 : 34일 / 286일 (최대 +68)
✓/✗ 역전      : 3일 — 전부 ✗ → ✓
   2026-07-06  섭취 1854 / 유효목표 1852 → 1920
   2026-07-23  섭취 1725 / 유효목표 1680 → 1748
   2026-07-27  섭취 2000 / 유효목표 1999 → 2067
```

세 날 모두 **1~45kcal 차이로 아슬아슬하게 ✗였던 날**이다.

### 이 수정은 과거를 바꾸는 게 아니라 **되돌린다**

`applyAdaptive`는 보정을 적용할 때 `adaptiveOn: true`도 함께 켠다(`App.jsx:883`).
즉 2026-07-03에 +68을 적용한 시점부터 적응형은 켜져 있었고, **그 3일에 화면은 실제로 ✓를
보여주고 있었다.** 나중에 토글을 끄는 순간 그 판정들이 소급해서 ✗로 바뀐 것이다.
수정은 그때 보던 ✓를 복원한다.

### 무엇을 바꿨나

끄기의 의미를 "이력 무시"에서 "**오늘부터** 보정 0"으로 고쳤다.

- `effectiveTdeeHistory(history, adaptiveOn, todayStr)` (utils.js) — 꺼져 있으면 과거 항목은
  그대로 두고 오늘자 0만 얹는다. **오늘의 목표는 이전과 동일하다**(실측 확인: 1541 → 1541).
- `setAdaptiveOn(false)` — 이력에 `{from: 오늘, adjust: 0}`을 **기록**한다. 읽는 쪽 보정에만
  기대면 끈 시점이 데이터에 남지 않아 기기마다 오늘이 달라진다.

**읽는 쪽 보정을 함께 둔 이유**: 토글을 끈 시점이 기록돼 있지 않은 기존 데이터가 이미 있다.
그것을 고치려면 앱을 여는 것만으로 저장이 일어나야 하는데(기기마다, 조용히), 파괴적 소급
수정은 하지 않는다. 대신 읽을 때마다 같은 결론을 내게 하고, 토글을 실제로 만지는 순간
진짜로 기록되어 그때부터 기기 간에도 일치한다.

### 검증

- 골든셋·297일 회귀 포함 전체 통과 — 이력이 없는 데이터는 출력이 바이트 단위로 동일
- 실데이터에 **실제 구현**(스크립트 모델이 아니라 `effectiveTdeeHistory`)을 적용해 예측한
  3일이 그대로 재현되고 오늘의 목표가 불변임을 확인
