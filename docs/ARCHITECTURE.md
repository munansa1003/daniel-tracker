...
16. **`.env.example`이 env 집합의 단일 출처다**(2026-08 감사 R-31에서 동기화 완료 — `IMPORT_*`·
    `INBODY_*`·`SHARE_TEST_TOKEN` 포함). env를 새로 읽는 코드를 넣으면 **여기에도 반드시 추가**한다.
    대조 방법: `grep -rhoP 'process\.env\.\K[A-Z_0-9]+' api/`에서 Vercel 자동 주입분
    (`VERCEL_*`·`NODE_ENV`)을 뺀 전부가 `.env.example`에 있어야 한다
17. **`CRON_SECRET`은 필수다** — 없으면 예약 푸시가 503으로 **정지한다**(2026-08 감사 R-40).
    옛 코드는 값이 없으면 검문을 건너뛰어 누구나 구독자 전원에게 푸시를 쏠 수 있었다. 열어두는 것보다
    멈추는 쪽이 안전하다는 판단이므로, "밤 8시 푸시가 안 온다"의 1순위 확인 대상은 이 env다
18. **자동 확정분과 손확정분의 잠금은 서로 다르다** — 카드에서 사용자가 확정하면 `draftToRecord`가
    `auto`를 붙이지 않아 그 날짜는 **영구 잠금**이 된다(이후 클라우드가 복구돼도 자동 갱신 대상이 아님).
    자동 확정분(`auto:true`)만 더 최신 `sampleTs`로 교체된다. B채널 폴백으로 며칠을 손확정했다면
    그 날짜들은 영영 수동 관리 대상이라는 뜻이다 — 의도된 동작이나 되돌릴 방법이 없다(감사 R-15)
19. **귀속일 기준이 경로마다 다르다** — 운동은 시작 시각의 **로컬 날짜**(오프셋 포함 문자열 앞 10자),
    체성분은 `IMPORT_TZ_OFFSET`(기본 +09:00) 기준, 크론의 "오늘"은 **KST 고정**, 앱의 "오늘"은
    **기기 현지 시각**이다. 국내에서는 넷이 일치하지만 해외(비-JST)에서는 같은 운동이 경로별로
    다른 날짜에 귀속될 수 있다(감사 R-17). 해외 사용 계획이 생기면 기준을 한 곳으로 모아야 한다
20. **`IMPORT_TZ_OFFSET`을 바꾸면 이미 확정된 날이 새 초안으로 되돌아올 수 있다** — 사서함 키가
    `date|sampleTs|source`인데 `sampleTs`는 epoch라 오프셋이 바뀌면 **같은 측정의 키가 달라진다**
    (`date`·`hour`는 그대로다). 새 키는 seen 도장이 없으니 새 항목으로 들어온다. 값을 바꿀 일이
    생기면 그 시점 이후 측정에만 적용되도록 컷오버와 함께 다루고, 되돌아온 초안은 폐기하면 된다
    (폐기는 tombstone으로 기억되므로 다시 올라오지 않는다 — 감사 R-45·R-02)
